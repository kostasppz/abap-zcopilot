...
| analyzer-core: 5 S/4HANA rules | ✅ complete | Simplified data-model access, obsolete declarations, native SQL and database hints; every finding includes migration guidance. |
| analyzer-core: 6 Clean ABAP rules | ✅ complete | Modern syntax, method-oriented design, nesting and boolean clarity; every finding includes a preview-only suggestion. |
...
| eclipse-plugin 0.5.0 | ✅ complete | RunPod service, encrypted token storage, all/category-only analysis commands, Copilot, live/on-save analysis, editor annotations, preview-only corrections and once-per-version Welcome. |
...
- `mvn clean verify` (default profile): **passing in CI for 0.5.0**; the coverage
